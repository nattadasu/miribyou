...
export function parseHover(html: string): HoverData {
  ...
  const synopsis = $("div[style*='margin-top: 8px']").clone().find("a").remove().end().text().trim();

  const extractList = (label: string, type: "anime" | "manga" = "anime"): MalUrl[] => {
  ...
  const demographics = extractList("Demographic:") || extractList("Demographics:");

  const getInfo = (label: string) => {
    const section = $(`.dark_text:contains('${label}')`);
    if (!section.length) return null;
    let text = section[0].nextSibling?.nodeType === 3 ? section[0].nextSibling.nodeValue?.trim() : null;
    if (!text) {
  ...
  const scored_by = scoredByMatch ? parseInt(scoredByMatch[1].replace(/,/g, "")) : null;

  const rankText = getInfo("Ranked:");
  const rank = rankText?.startsWith("#") ? parseInt(rankText.substring(1)) : null;

  const popularityText = getInfo("Popularity:");
  const popularity = popularityText?.startsWith("#") ? parseInt(popularityText.substring(1)) : null;
